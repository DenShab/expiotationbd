...
console.log(btn);
    ...
    if (formCount < 1000) {
        ...
        var row = $(".item_" + prefix + ":last").clone(false).get(0);

        // Insert it after the last form
        $(row).removeAttr('id').hide().insertAfter(".item_" + prefix + ":last").slideDown(300);
...
function deleteForm(btn, prefix) {
      ...
      if (formCount > 1) {
          ...
          if( goto_id ){
            $.ajax({
                ...
                success: function (data) {
                  $(btn).parents('.item_' + prefix).remove();
                },
                type: 'GET'
            });
          }else{
          ...
          var forms = $('.item_' + prefix); // Get all the forms
